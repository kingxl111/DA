...
pair<vector<uint8_t>, int> BWT(const vector<uint8_t>& input) {
    ...
    for (int i = 0; i < n; ++i) {
        rotations.push_back(string(input.begin() + i, input.end()) + string(input.begin(), input.begin() + i));
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (rotations[i] == string(input.begin(), input.end())) {
            primaryIndex = i;
        }
    }
...
vector<uint8_t> ReverseBWT(const vector<uint8_t>& bwt, int primaryIndex) {
    int n = bwt.size();
    vector<pair<uint8_t, int>> sortedBWT(n);
    ...
    for (int i = 0; i < n; ++i) {
        result[i] = sortedBWT[idx].first;
        idx = sortedBWT[idx].second;
    }
